...
public class AdditionalDistanceConstraintExample {
  ...
  static class DistanceUpdater implements StateUpdater, ActivityVisitor {
    ...
    // public DistanceUpdater(StateFactory.StateId distanceStateId, StateManager stateManager,
    // VehicleRoutingTransportCostsMatrix costMatrix) { //v1.3.1
    public DistanceUpdater(StateId distanceStateId, StateManager stateManager,
        VehicleRoutingTransportCostsMatrix transportCosts) { // head of development - upcoming
                                                             // release (v1.4)
    ...
    public void finish() {
      ...
      // stateManager.putTypedRouteState(vehicleRoute,distanceStateId,Double.class,distance);
      // //v1.3.1
      stateManager.putRouteState(vehicleRoute, distanceStateId, distance); // head of development -
                                                                           // upcoming release
                                                                           // (v1.4)
    }
  ...
  static class DistanceConstraint implements HardActivityConstraint {
    ...
    // DistanceConstraint(double maxDistance, StateFactory.StateId distanceStateId, StateManager
    // stateManager, VehicleRoutingTransportCostsMatrix costsMatrix) { //v1.3.1
    DistanceConstraint(double maxDistance, StateId distanceStateId, StateManager stateManager,
        VehicleRoutingTransportCostsMatrix transportCosts) { // head of development - upcoming
                                                             // release (v1.4)
    ...
    public ConstraintsStatus fulfilled(JobInsertionContext context, TourActivity prevAct,
        TourActivity newAct, TourActivity nextAct, double v) {
      double additionalDistance = getDistance(prevAct, newAct) + getDistance(newAct, nextAct)
          - getDistance(prevAct, nextAct);
      Double routeDistance =
          stateManager.getRouteState(context.getRoute(), distanceStateId, Double.class);
  ...
  public static void main(String[] args) {
    ...
    // builds a matrix based on euclidean distances; t_ij = euclidean(i,j) / 2; d_ij =
    // euclidean(i,j);
    ...
    StateManager stateManager = new StateManager(vrp); // head of development - upcoming release
                                                       // (v1.4)

    StateId distanceStateId = stateManager.createStateId("distance"); // head of development -
                                                                      // upcoming release (v1.4)
    stateManager.addStateUpdater(new DistanceUpdater(distanceStateId, stateManager, costMatrix));

    ConstraintManager constraintManager = new ConstraintManager(vrp, stateManager);
    constraintManager.addConstraint(
        new DistanceConstraint(120., distanceStateId, stateManager, costMatrix),
  ...
  private static VehicleRoutingTransportCostsMatrix createMatrix(
