 ...
 * <p>
 * It has the hard requirements that no two different products can be transported in the same
 * vehicle at the same time. This might be important if products require different temperatures. For
 * example, if a vehicle transports apples then no bananas can be loaded (and the other way around).
 * Once all apples have been unloaded, bananas can be loaded.
 * <p>
 * See also the discussion here:
 * https://groups.google.com/forum/#!topic/jsprit-mailing-list/2JQqY4loC0U
 */
public class MultipleProductsWithLoadConstraintExample {
  ...
  static class BananasFirst implements HardActivityConstraint {
    ...
    @Override
    public ConstraintsStatus fulfilled(JobInsertionContext jobInsertionContext,
        TourActivity prevActivity, TourActivity newActivity, TourActivity nextActivity,
        double departureTimeAtPrevActivity) {
  ...
  static class NoBananasANDApplesConstraint implements HardActivityConstraint {
    ...
    public ConstraintsStatus fulfilled(JobInsertionContext jobInsertionContext,
      ...
      if (isDelivery(newAct)) {
        if ((isAppleDelivery(newAct) && hasBananasInVehicle(loadAtPrevAct))
          ...
          return ConstraintsStatus.NOT_FULFILLED_BREAK; // if so constraint is broken forever ->
                                                        // break here
        }
        return ConstraintsStatus.FULFILLED;
      }
    ...
    private Capacity getLoadAtPreviousAct(TourActivity prevAct) {
      // Capacity prevLoad = stateManager.getActivityState(prevAct, StateFactory.LOAD,
      // Capacity.class); //v1.3.1
      Capacity prevLoad =
          stateManager.getActivityState(prevAct, InternalStates.LOAD, Capacity.class); // 1.3.2-SNAPSHOT
                                                                                       // & upcoming
                                                                                       // release
                                                                                       // v1.4
      if (prevLoad != null)
  ...
  public static void main(String[] args) {
    ...
    VehicleType type = VehicleTypeImpl.Builder.newInstance("type")
        .addCapacityDimension(BANANAS_DIMENSION_INDEX, 10)
        .addCapacityDimension(APPLES_DIMENSION_INDEX, 20).build();
    VehicleImpl vehicle = VehicleImpl.Builder.newInstance("vehicle")
        .setStartLocation(loc(Coordinate.newInstance(0, 0))).setType(type).build();

    Shipment bananas =
        Shipment.Builder.newInstance("bananas_1").addSizeDimension(BANANAS_DIMENSION_INDEX, 1)
            .setPickupLocation(loc(Coordinate.newInstance(1, 8)))
            .setDeliveryLocation(loc(Coordinate.newInstance(10, 8))).build();

    Shipment bananas_2 =
        Shipment.Builder.newInstance("bananas_2").addSizeDimension(BANANAS_DIMENSION_INDEX, 1)
            .setPickupLocation(loc(Coordinate.newInstance(2, 8)))
            .setDeliveryLocation(loc(Coordinate.newInstance(11, 8))).build();

    Shipment bananas_3 =
        Shipment.Builder.newInstance("bananas_3").addSizeDimension(BANANAS_DIMENSION_INDEX, 1)
            .setPickupLocation(loc(Coordinate.newInstance(3, 8)))
            .setDeliveryLocation(loc(Coordinate.newInstance(12, 8))).build();

    Shipment apples =
        Shipment.Builder.newInstance("apples_1").addSizeDimension(APPLES_DIMENSION_INDEX, 1)
            .setPickupLocation(loc(Coordinate.newInstance(1, 6)))
            .setDeliveryLocation(loc(Coordinate.newInstance(10, 12))).build();

    Shipment apples_2 =
        Shipment.Builder.newInstance("apples_2").addSizeDimension(APPLES_DIMENSION_INDEX, 1)
            .setPickupLocation(loc(Coordinate.newInstance(1, 5)))
            .setDeliveryLocation(loc(Coordinate.newInstance(10, 11))).build();

    VehicleRoutingProblem vrp = VehicleRoutingProblem.Builder.newInstance()
        .setFleetSize(VehicleRoutingProblem.FleetSize.INFINITE).addVehicle(vehicle).addJob(bananas)
        .addJob(apples).addJob(bananas_2).addJob(bananas_3).addJob(apples_2).build();

    StateManager stateManager = new StateManager(vrp);
    ConstraintManager constraintManager = new ConstraintManager(vrp, stateManager);
    constraintManager.addConstraint(new NoBananasANDApplesConstraint(stateManager),
    ...
    new GraphStreamViewer(vrp, Solutions.bestOf(solutions)).labelWith(GraphStreamViewer.Label.ID)
