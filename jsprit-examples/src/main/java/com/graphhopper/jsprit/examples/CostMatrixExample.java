...
public class CostMatrixExample {
  ...
  public static void main(String[] args) {
    ...
    Service s3 = Service.Builder.newInstance("3").addSizeDimension(0, 1)
        .setLocation(Location.newInstance("3")).build();


    /*
     * Assume the following symmetric distance-matrix from,to,distance 0,1,10.0 0,2,20.0 0,3,5.0
     * 1,2,4.0 1,3,1.0 2,3,2.0
     *
    ...
    VehicleRoutingProblem vrp = VehicleRoutingProblem.Builder.newInstance()
        .setFleetSize(FleetSize.INFINITE).setRoutingCost(costMatrix).addVehicle(vehicle).addJob(s1)
        .addJob(s2).addJob(s3).build();
