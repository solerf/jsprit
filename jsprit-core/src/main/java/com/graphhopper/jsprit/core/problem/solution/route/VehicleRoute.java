...
public class VehicleRoute {
   ...
   * <p>
   * <p>
   * An empty route has an empty list of tour-activities, no driver (DriverImpl.noDriver()) and no
   * vehicle (VehicleImpl.createNoVehicle()).
   *
   * @return empty route
   */
  ...
  public static class Builder {
    ...
    private final Map<Shipment, TourActivity> openActivities =
        new HashMap<Shipment, TourActivity>();
    ...
    private JobActivityFactory jobActivityFactory = new JobActivityFactory() {
      ...
      private final TourShipmentActivityFactory shipmentActivityFactory =
    ...
    private Builder(Vehicle vehicle, Driver driver) {
      ...
      this.driver = driver;
      start =
          new Start(vehicle.getStartLocation(), vehicle.getEarliestDeparture(), Double.MAX_VALUE);
      start.setEndTime(vehicle.getEarliestDeparture());
      end = new End(vehicle.getEndLocation(), 0.0, vehicle.getLatestArrival());
    }

    /**
     * Sets the departure-time of the route, i.e. which is the time the vehicle departs from
     * start-location.
     * <p>
     * <p>
     * <b>Note</b> that departureTime cannot be lower than earliestDepartureTime of vehicle.
     *
    ...
    /**
     * Adds a service to this route. Activity is initialized with .getTimeWindow(). If you want to
     * explicitly set another time window use .addService(Service service, TimeWindow timeWindow)
     * <p>
     * <p>
     * This implies that for this service a serviceActivity is created with
     * {@link TourActivityFactory} and added to the sequence of tourActivities.
     * <p>
     * <p>
    ...
    private Builder addBreakInternally(Break currentBreak, TimeWindow timeWindow,
     ...
     * @return the builder
     * @throws IllegalArgumentException if method has already been called with the specified
     *         shipment.
     */
     ...
     * @return builder
     * @throws IllegalArgumentException if specified shipment has not been picked up yet (i.e.
     ...
     * @throws IllegalArgumentException if there are still shipments that have been picked up though
     *         but not delivery.
     */
    public VehicleRoute build() {
      if (!openShipments.isEmpty()) {
        throw new IllegalArgumentException(
            "there are still shipments that have not been delivered yet.");
      }
      if (!vehicle.isReturnToDepot()) {
        if (!tourActivities.isEmpty()) {
          end.setLocation(tourActivities.getActivities()
              .get(tourActivities.getActivities().size() - 1).getLocation());
        }
      }
   ...
   * <p>
   * startActivity is initialized with the start-location of the specified vehicle
   * (<code>vehicle.getStartLocationId()</code>). the time-window of this activity is initialized
   * such that [<code>startActivity.getTheoreticalEarliestOperationStartTime()</code> =
   ...
   * (<code>vehicle.getEndLocationId()</code>). The time-window of the endActivity is initialized
   * such that [<code>endActivity.getTheoreticalEarliestOperationStartTime()</code> =
   ...
   * max{<code>vehicle.getEarliestDeparture()</code>, <code>vehicleDepTime</code>}. thus,
   * <code>vehicle.getEarliestDeparture()</code> is a physical constraint that has to be met.
   *
  ...
  private void setStartAndEnd(Vehicle vehicle, double vehicleDepTime) {
    if (!(vehicle instanceof VehicleImpl.NoVehicle)) {
      if (start == null && end == null) {
        start = new Start(vehicle.getStartLocation(), vehicle.getEarliestDeparture(),
            vehicle.getLatestArrival());
        end = new End(vehicle.getEndLocation(), vehicle.getEarliestDeparture(),
            vehicle.getLatestArrival());
      }
  ...
  public String toString() {
    return "[start=" + start + "][end=" + end + "][departureTime=" + start.getEndTime()
        + "][vehicle=" + vehicle + "][driver=" + driver + "][nuOfActs="
        + tourActivities.getActivities().size() + "]";
  }
