...
class JobNeighborhoodsOptimized implements JobNeighborhoods {
  ...
  public JobNeighborhoodsOptimized(VehicleRoutingProblem vrp, JobDistance jobDistance,
      int capacity) {
    super();
    this.vrp = vrp;
  ...
  private void calculateDistancesFromJob2Job() {
    ...
    for (Job job_i : vrp.getJobsInclusiveInitialJobsInRoutes().values()) {
      ...
      List<ReferencedJob> jobList =
          new ArrayList<ReferencedJob>(vrp.getJobsInclusiveInitialJobsInRoutes().values().size());
