...
class Scorer {
  ...
  static double score(Job unassignedJob, InsertionData best, InsertionData secondBest,
    ...
    double score;
    if (secondBest == null) { // either there is only one vehicle or there are more vehicles, but
                              // they cannot load unassignedJob
      ...
      // alternatives
      score = (11 - unassignedJob.getPriority()) * (Integer.MAX_VALUE - best.getInsertionCost())
          + scoringFunction.score(best, unassignedJob);
    } else {
      score = (11 - unassignedJob.getPriority())
