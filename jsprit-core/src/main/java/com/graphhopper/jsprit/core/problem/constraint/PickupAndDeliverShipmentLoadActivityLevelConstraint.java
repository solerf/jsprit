 ...
 * that is MUST be visited. It also assumes that pd-activities are visited in the order they occur
 * in a tour.
 *
 * @author schroeder
 */
public class PickupAndDeliverShipmentLoadActivityLevelConstraint implements HardActivityConstraint {
   ...
   * <p>
   * <p>
   * This is critical to consistently calculate pd-problems with capacity constraints. Critical
   * means that is MUST be visited. It also assumes that pd-activities are visited in the order they
   * occur in a tour.
   *
   * @param stateManager the stateManager
   */
  public PickupAndDeliverShipmentLoadActivityLevelConstraint(
  ...
  public ConstraintsStatus fulfilled(JobInsertionContext iFacts, TourActivity prevAct,
    ...
    if (newAct instanceof PickupShipment) {
      if (!Capacity.addup(loadAtPrevAct, newAct.getSize())
          .isLessOrEqual(iFacts.getNewVehicle().getType().getCapacityDimensions())) {
        return ConstraintsStatus.NOT_FULFILLED;
      }
    }
    if (newAct instanceof DeliverShipment) {
      if (!Capacity.addup(loadAtPrevAct, Capacity.invert(newAct.getSize()))
          .isLessOrEqual(iFacts.getNewVehicle().getType().getCapacityDimensions()))
