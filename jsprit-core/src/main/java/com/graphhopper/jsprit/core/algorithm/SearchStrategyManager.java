...
public class SearchStrategyManager {
  ...
  private final List<SearchStrategyListener> searchStrategyListeners =
      new ArrayList<SearchStrategyListener>();
  ...
  public void addStrategy(SearchStrategy strategy, double weight) {
    ...
    if (id2index.containsKey(strategy.getId())) {
      throw new IllegalStateException("strategyId " + strategy.getId()
   ...
   * @throws java.lang.IllegalStateException if randomNumberGenerator is null OR no search strategy
   *         can be found
   */
  public SearchStrategy getRandomStrategy() {
    if (random == null)
      throw new IllegalStateException(
          "randomizer is null. make sure you set random object correctly");
