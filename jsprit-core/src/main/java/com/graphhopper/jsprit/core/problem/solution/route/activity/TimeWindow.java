...
public class TimeWindow {
  ...
  public TimeWindow(double start, double end) {
    ...
    if (start < 0.0 || end < 0.0)
      throw new IllegalArgumentException("neither time window start nor end must be < 0.0: "
          + "[start=" + start + "][end=" + end + "]");
    if (end < start)
      throw new IllegalArgumentException("time window end cannot be smaller than its start: "
          + "[start=" + start + "][end=" + end + "]");
    this.start = start;
    this.end = end;
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (Double.doubleToLongBits(end) != Double.doubleToLongBits(other.end))
      return false;
    return Double.doubleToLongBits(start) == Double.doubleToLongBits(other.start);
  }
