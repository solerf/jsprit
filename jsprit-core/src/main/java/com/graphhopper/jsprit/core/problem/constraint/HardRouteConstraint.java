...
public interface HardRouteConstraint extends HardConstraint {
   ...
   *
   * @param insertionContext provides context information about inserting a new job, i.e. the new
   *        job (<code>insertionContext.getJob()</code>), the route where the new job should be
   *        inserted (<code>insertionContext.getRoute()</code>), the new vehicle that should operate
   *        the route plus the new job (<code>insertionContext.getNewVehicle()</code>) and the new
   *        departure time at this vehicle's start location
   *        (<code>insertionContext.getNewDepartureTime()</code>).
