...
public class UpdateFutureWaitingTimes implements ReverseActivityVisitor, StateUpdater {
  ...
  public void visit(TourActivity activity) {
    states.putInternalTypedActivityState(activity, route.getVehicle(),
        InternalStates.FUTURE_WAITING, futureWaiting);
    if (!(activity instanceof BreakActivity)) {
      futureWaiting +=
          Math.max(activity.getTheoreticalEarliestOperationStartTime() - activity.getArrTime(), 0);
    }
  }
