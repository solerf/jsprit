...
/**
 * Ruin strategy that ruins current solution randomly. I.e. customer are removed randomly from
 * current solution.
 *
...
public final class RuinWorst extends AbstractRuinStrategy {
   ...
   * <p>
   * <p>
   * The number of jobs is calculated as follows: Math.ceil(vrp.getJobs().values().size() *
  ...
  private void ruin(Collection<VehicleRoute> vehicleRoutes, int nOfJobs2BeRemoved,
  ...
  private double savings(VehicleRoute route, TourActivity actBefore, TourActivity actToEval,
      TourActivity act) {
    double savings = c(actBefore, actToEval, route.getVehicle())
        + c(actToEval, act, route.getVehicle()) - c(actBefore, act, route.getVehicle());
    return Math.max(0, savings + noiseMaker.makeNoise());
  }

  private double c(TourActivity from, TourActivity to, Vehicle vehicle) {
    return vrp.getTransportCosts().getTransportCost(from.getLocation(), to.getLocation(),
        from.getEndTime(), DriverImpl.noDriver(), vehicle);
  }
