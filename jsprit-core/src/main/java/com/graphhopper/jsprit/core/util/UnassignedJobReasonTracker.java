...
public class UnassignedJobReasonTracker implements JobUnassignedListener {
  ...
  public static String getMostLikelyFailedConstraintName(Frequency failedConstraintNamesFrequency) {
    if (failedConstraintNamesFrequency == null
        || failedConstraintNamesFrequency.getUniqueCount() == 0)
      return NO_REASON;
    Iterator<Map.Entry<Comparable<?>, Long>> entryIterator =
        failedConstraintNamesFrequency.entrySetIterator();
    long maxCount = 0;
  ...
  public UnassignedJobReasonTracker() {
    ...
    codesToHumanReadableReason.put(4,
        "cannot be assigned due to max distance constraint of vehicle");
  ...
  public void informJobUnassigned(Job unassigned, Collection<String> failedConstraintNames) {
    ...
    for (String r : failedConstraintNames) {
      if (failedConstraintNamesToBeIgnored.contains(r))
        continue;
  ...
  public void put(String simpleNameOfFailedConstraint, int code, String reason) {
    if (code <= 20)
      throw new IllegalArgumentException(
          "first 20 codes are reserved internally. choose a code > 20");
  ...
  /**
   * For each job id, it returns frequency distribution of failed constraints (simple name of
   * constraint) in an unmodifiable map.
   *
   * @return
   */
  @Deprecated
  ...
  /**
   * Returns an unmodifiable map of constraint names (simple name of constraint) and reason code
   * pairs.
   *
   * @return
   */
  ...
  /**
   * Returns the most likely reason code i.e. the reason (failed constraint) being observed most
   * often.
   * <p>
   * 1 --> "cannot serve required skill 2 --> "cannot be visited within time window" 3 --> "does not
   * fit into any vehicle due to capacity" 4 --> "cannot be assigned due to max distance constraint
   * of vehicle"
   *
   * @param jobId
   * @return
  ...
  public String getMostLikelyReason(String jobId) {
    if (!this.failedConstraintNamesFrequencyMapping.containsKey(jobId))
      return codesToHumanReadableReason.get(-1);
    Frequency reasons = this.failedConstraintNamesFrequencyMapping.get(jobId);
    ...
    if (code == -1)
      ...
      return codesToHumanReadableReason.get(code);
  }
