 ...
 * <p>
 * <p>
 * To construct the problem, use VehicleRoutingProblem.Builder. Get an instance of this by using the
 * static method VehicleRoutingProblem.Builder.newInstance().
 * <p>
 * <p>
 * By default, fleetSize is INFINITE, transport-costs are calculated as euclidean-distance
 * (CrowFlyCosts), and activity-costs are set to zero.
 *
...
public class VehicleRoutingProblem {
  ...
  public static final class Builder {
    ...
    private final DefaultTourActivityFactory serviceActivityFactory =
     ...
     * <p>
     * <p>
     * FleetSize is either FleetSize.INFINITE or FleetSize.FINITE. By default it is
     * FleetSize.INFINITE.
     *
     * @param fleetSize the fleet size used in this problem. it can either be FleetSize.INFINITE or
     *        FleetSize.FINITE
     * @return this builder
     */
     ...
     * <p>
     * <p>
     * Note that job.getId() must be unique, i.e. no job (either it is a shipment or a service) is
     * allowed to have an already allocated id.
     *
     * @param job job to be added
     * @return this builder
     * @throws IllegalStateException if job is neither a shipment nor a service, or jobId has
     *         already been added.
     */
    ...
    public Builder addJob(AbstractJob job) {
      if (tentativeJobs.containsKey(job.getId()))
        throw new IllegalArgumentException(
            "The vehicle routing problem already contains a service or shipment with id "
                + job.getId()
                + ". Please make sure you use unique ids for all services and shipments.");
      ...
      for (Vehicle v : uniqueVehicles) {
        if (v.getBreak() != null) {
          if (!uniqueBreakIds.add(v.getBreak().getId()))
            throw new IllegalArgumentException(
                "The vehicle routing roblem already contains a vehicle break with id "
                    + v.getBreak().getId() + ". Please choose unique ids for each vehicle break.");
          hasBreaks = true;
          List<AbstractActivity> breakActivities =
              jobActivityFactory.createActivities(v.getBreak());
          if (breakActivities.isEmpty())
            throw new IllegalArgumentException(
                "At least one activity for break needs to be created by activityFactory.");
    ...
    private void addJobToFinalMap(Job job) {
      if (jobs.containsKey(job.getId())) {
        logger.warn("The job " + job
            + " has already been added to the job list. This overrides the existing job.");
      }
      addLocationToTentativeLocations(job);
    ...
    public Builder addVehicle(AbstractVehicle vehicle) {
      if (addedVehicleIds.contains(vehicle.getId())) {
        throw new IllegalArgumentException(
            "The vehicle routing problem already contains a vehicle with id " + vehicle.getId()
                + ". Please choose unique ids for each vehicle.");
      } else
      ...
      if (typeKeyIndices.containsKey(vehicle.getVehicleTypeIdentifier())) {
        vehicle.getVehicleTypeIdentifier()
            .setIndex(typeKeyIndices.get(vehicle.getVehicleTypeIdentifier()));
      } else {
      ...
      if (!vehicleTypes.containsKey(vehicle.getType().getTypeId())) {
        ...
        if (!vehicle.getType().equals(existingType)) {
          throw new IllegalArgumentException("A type with type id " + vehicle.getType().getTypeId()
              + " already exists. However, types are different. Please use unique vehicle types only.");
        }
      }
      ...
      if (!vehicle.getEndLocation().getId().equals(startLocationId)) {
        ...
        // tentative_coordinates.put(vehicle.getEndLocation().getId(),
        // vehicle.getEndLocation().getCoordinate());
      }
      return this;
    }
    ...
    public VehicleRoutingProblem build() {
      ...
      if (hasBreaks && fleetSize.equals(FleetSize.INFINITE))
        throw new UnsupportedOperationException(
            "Breaks are not yet supported when dealing with infinite fleet. Either set it to finite or omit breaks.");
  ...
  public String toString() {
    return "[problemId=" + problemId + "][fleetSize=" + fleetSize + "][#jobs=" + jobs.size()
        + "][#vehicles=" + vehicles.size() + "][#vehicleTypes=" + vehicleTypes.size() + "]["
  ...
  // public Map<Job,List<AbstractActivity>> getActivityMap() { return
