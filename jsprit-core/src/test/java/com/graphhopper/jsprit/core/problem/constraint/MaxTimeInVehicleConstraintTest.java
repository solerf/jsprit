...
public class MaxTimeInVehicleConstraintTest {
  ...
  private void ini(double maxTimeShipment, double maxTimeDelivery, double maxTimePickup) {
    ...
    d2 = Delivery.Builder.newInstance("d2").setMaxTimeInVehicle(maxTimeDelivery)
    ...
    vrp = VehicleRoutingProblem.Builder.newInstance().addJob(d1).addJob(s1).addJob(d2).addJob(p1)
        .addJob(p2).addVehicle(v).build();
  ...
  public void shiftOfExistingShipmentsShouldWork() {
    ...
    Shipment s2 = Shipment.Builder.newInstance("s2").setPickupLocation(Location.newInstance(20, 0))
        .setPickupServiceTime(10).setDeliveryLocation(Location.newInstance(40, 0))
        .setMaxTimeInVehicle(20).build();

    VehicleRoutingProblem vrp =
        VehicleRoutingProblem.Builder.newInstance().addJob(s1).addJob(s2).addVehicle(v).build();

    VehicleRoute route = VehicleRoute.Builder.newInstance(v)
        .setJobActivityFactory(vrp.getJobActivityFactory()).addPickup(s1).addDelivery(s1).build();
    ...
    UpdateMaxTimeInVehicle updater = new UpdateMaxTimeInVehicle(stateManager, minSlackId,
        vrp.getTransportCosts(), vrp.getActivityCosts(), openJobsId);
    stateManager.addStateUpdater(updater);
    stateManager
        .addStateUpdater(new UpdateActivityTimes(vrp.getTransportCosts(), vrp.getActivityCosts()));
    stateManager.informInsertionStarts(Arrays.asList(route), new ArrayList<Job>());

    MaxTimeInVehicleConstraint constraint = new MaxTimeInVehicleConstraint(vrp.getTransportCosts(),
        vrp.getActivityCosts(), minSlackId, stateManager, vrp, openJobsId);
    ...
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, route.getStart(), acts.get(0), route.getActivities().get(0), 0));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED,
        constraint.fulfilled(c, act(route, 0), acts.get(0), act(route, 1), 20));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, act(route, 1), acts.get(0), route.getEnd(), 40));
    ...
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, acts.get(0), acts.get(1), act(route, 0), 30));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, act(route, 0), acts.get(1), act(route, 1), 30));
  }
  ...
  public void insertingDeliveryAtAnyPositionShouldWork() {
    ...
    UpdateMaxTimeInVehicle updater = new UpdateMaxTimeInVehicle(stateManager, latestStartId,
        vrp.getTransportCosts(), vrp.getActivityCosts(), openJobsId);
    stateManager.addStateUpdater(updater);
    stateManager
        .addStateUpdater(new UpdateActivityTimes(vrp.getTransportCosts(), vrp.getActivityCosts()));
    ...
    MaxTimeInVehicleConstraint constraint = new MaxTimeInVehicleConstraint(vrp.getTransportCosts(),
        vrp.getActivityCosts(), latestStartId, stateManager, vrp, openJobsId);
    ...
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, route.getStart(), acts.get(0), route.getActivities().get(0), 0));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED, constraint.fulfilled(c,
        route.getActivities().get(0), acts.get(0), route.getActivities().get(1), 10));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED, constraint.fulfilled(c,
        route.getActivities().get(1), acts.get(0), route.getActivities().get(2), 20));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, route.getActivities().get(2), acts.get(0), route.getEnd(), 40));
  ...
  public void insertingD2JustAfterStartShouldWork() {
    ...
    Assert.assertEquals("inserting d2 just after start should work",
        HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, route.getStart(), acts.get(0), route.getActivities().get(0), 0));
  }
  ...
  public void insertingD2AfterFirstDeliveryShouldWork() {
    ...
    Assert.assertEquals("inserting d2 after first delivery should work",
  ...
  public void insertingDeliveryInBetweenShipmentShouldFail() {
    ...
    Assert.assertEquals(
        ...
        HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED, constraint.fulfilled(c,
            route.getActivities().get(1), acts.get(0), route.getActivities().get(2), 20));
    Assert.assertEquals("inserting d2 at end should fail",
  ...
  public void insertingPickupShipmentAtAnyPositionShouldWork() {
    ...
    VehicleRoute r = VehicleRoute.Builder.newInstance(v)
        .setJobActivityFactory(vrp.getJobActivityFactory()).addDelivery(d1).addDelivery(d2).build();
    ...
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, r.getStart(), acts.get(0), r.getActivities().get(0), 0));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED, constraint.fulfilled(c,
        r.getActivities().get(0), acts.get(0), r.getActivities().get(1), 10));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, r.getActivities().get(1), acts.get(0), r.getEnd(), 40));
  }
  ...
  public void insertingPickupShipmentShouldWork() {
    ...
    VehicleRoute r = VehicleRoute.Builder.newInstance(v)
        .setJobActivityFactory(vrp.getJobActivityFactory()).addPickup(p1).addDelivery(d2).build();
    ...
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED, constraint.fulfilled(c,
        r.getActivities().get(0), acts.get(0), r.getActivities().get(1), 10));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, r.getActivities().get(1), acts.get(0), r.getEnd(), 30));
  ...
  public void insertingPickupShipmentShouldWork2() {
    ...
    Assert.assertEquals(
        ...
        HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED,
        constraint.fulfilled(c, r.getStart(), acts.get(0), r.getActivities().get(0), 0));
    Assert.assertEquals("pickup shipment can happen at second pos.",
        HardActivityConstraint.ConstraintsStatus.FULFILLED, constraint.fulfilled(c,
            r.getActivities().get(0), acts.get(0), r.getActivities().get(1), 10));
    Assert.assertEquals("d2 has been delivered so pickup shipment is possible",
        HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, r.getActivities().get(1), acts.get(0), r.getEnd(), 30));
  }
  ...
  public void testOpenRoutes() {
    /*
     * max time of deliveries and shipment should not be influenced at all by open routes when
     * pickups are supported it should matter
     */
  ...
  // VehicleRoute r =
  // VehicleRoute.Builder.newInstance(v).setJobActivityFactory(vrp.getJobActivityFactory())
  ...
  //
  // UpdateMaxTimeInVehicle updater = new UpdateMaxTimeInVehicle(stateManager, latestStartId,
  // vrp.getTransportCosts(), vrp.getActivityCosts());
  // stateManager.addStateUpdater(updater);
  // stateManager.informInsertionStarts(Arrays.asList(r), new ArrayList<Job>());
  //
  // MaxTimeInVehicleConstraint constraint = new MaxTimeInVehicleConstraint(vrp.getTransportCosts(),
  // vrp.getActivityCosts(), latestStartId, stateManager, vrp);
  // JobInsertionContext c = new JobInsertionContext(r, p2, v, r.getDriver(), 0.);
  // List<AbstractActivity> acts = vrp.getActivities(p2);
  // c.getAssociatedActivities().add(acts.get(0));
  //
  //
  // Assert.assertEquals("p2 cannot be done at first pos. due to its own max in-vehicle time
  // restriction",HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED, constraint.fulfilled(c,
  // r.getStart(), acts.get(0), r.getActivities().get(0), 0));
  // Assert.assertEquals("p2 cannot be done at second pos. due to its own max in-vehicle time
  // restriction",HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED, constraint.fulfilled(c,
  // r.getActivities().get(0), acts.get(0), r.getActivities().get(1), 10));
  // Assert.assertEquals("p2 cannot be done at third pos. due to its own max in-vehicle time
  // restriction", HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED, constraint.fulfilled(c,
  // r.getActivities().get(1), acts.get(0), r.getActivities().get(2), 20));
  // Assert.assertEquals("p2 can be done at last",
  // HardActivityConstraint.ConstraintsStatus.FULFILLED, constraint.fulfilled(c,
  ...
  public void whenPickupIsInsertedAt0_insertingDeliveryShipmentShouldFailWhereConstraintIsBroken() {
    ...
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, acts.get(0), acts.get(1), r.getActivities().get(0), 20));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED, constraint
        .fulfilled(c, r.getActivities().get(0), acts.get(1), r.getActivities().get(1), 30));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED,
        constraint.fulfilled(c, r.getActivities().get(1), acts.get(1), r.getEnd(), 40));
  }
  ...
  public void whenPickupIsInsertedAt1_insertingDeliveryShipmentShouldFailWhereConstraintIsBroken() {
    ...
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.FULFILLED,
        constraint.fulfilled(c, acts.get(0), acts.get(1), r.getActivities().get(1), 20));
    Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED,
        constraint.fulfilled(c, r.getActivities().get(1), acts.get(1), r.getEnd(), 40));
    // Assert.assertEquals(HardActivityConstraint.ConstraintsStatus.NOT_FULFILLED,
    // constraint.fulfilled(c, r.getActivities().get(1), acts.get(1), r.getEnd(), 40));
