...
public class RefuseCollectionWithCostsHigherThanTimesAndFiniteFleet_withTimeAndDistanceCosts_IT {
  ...
  private void readDemandQuantities(VehicleRoutingProblem.Builder vrpBuilder) throws IOException {
    BufferedReader reader = new BufferedReader(new InputStreamReader(
        getClass().getResourceAsStream("refuseCollectionExample_Quantities")));
    String line = null;
  ...
  private void readDistances(VehicleRoutingTransportCostsMatrix.Builder matrixBuilder)
    ...
    BufferedReader reader = new BufferedReader(
        new InputStreamReader(getClass().getResourceAsStream("refuseCollectionExample_Distances")));
    String line = null;
    boolean firstLine = true;
    while ((line = reader.readLine()) != null) {
      if (firstLine) {
      ...
      matrixBuilder.addTransportDistance(lineTokens[0], lineTokens[1],
          2. * Integer.parseInt(lineTokens[2]));
