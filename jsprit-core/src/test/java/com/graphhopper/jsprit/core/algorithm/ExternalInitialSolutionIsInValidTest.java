...
public class ExternalInitialSolutionIsInValidTest {
  ...
  public void itShouldSolveProblemWithIniSolutionExternallyCreated() {
    ...
    VehicleRoute route1 = VehicleRoute.Builder.newInstance(vehicle)
        .setJobActivityFactory(vrp.getJobActivityFactory()).addService(s1).build();

    vra.addInitialSolution(
        new VehicleRoutingProblemSolution("any-id", Collections.singletonList(route1), 20.));
