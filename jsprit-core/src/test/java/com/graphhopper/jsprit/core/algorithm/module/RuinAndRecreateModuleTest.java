...
public class RuinAndRecreateModuleTest {
  ...
  public void initialNumOfUnassignedShouldWorkCorrectly() {
    ...
    RuinAndRecreateModule module =
        new RuinAndRecreateModule("name", insertionStrategy, ruinStrategy);
    Collection<VehicleRoute> routes = new ArrayList<>();
    List<Job> unassigned = new ArrayList<>();
    for (int i = 0; i < 20; i++) {
      unassigned.add(mock(Job.class));
    }
    VehicleRoutingProblemSolution previousSolution =
        new VehicleRoutingProblemSolution("any-id", routes, unassigned, 0);
