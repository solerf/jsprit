...
public class JobInsertionConsideringFixCostsCalculatorTest {
  ...
  public void doBefore() {
    ...
    VehicleType smallType = VehicleTypeImpl.Builder.newInstance("smallType")
        .addCapacityDimension(0, 50).setFixedCost(50.0).build();
    when(small.getType()).thenReturn(smallType);

    medium = mock(Vehicle.class);
    VehicleType mediumType = VehicleTypeImpl.Builder.newInstance("mediumType")
        .addCapacityDimension(0, 100).setFixedCost(100.0).build();
    when(medium.getType()).thenReturn(mediumType);

    large = mock(Vehicle.class);
    VehicleType largeType = VehicleTypeImpl.Builder.newInstance("largeType")
        .addCapacityDimension(0, 400).setFixedCost(200.0).build();
    ...
    when(jobInsertionCosts.getInsertionData(route, job, medium, 0.0, null, Double.MAX_VALUE))
        .thenReturn(iData);
    when(jobInsertionCosts.getInsertionData(route, job, large, 0.0, null, Double.MAX_VALUE))
        .thenReturn(new InsertionData(0.0, 1, 1, large, null));
    ...
    when(stateGetter.getRouteState(route, InternalStates.MAXLOAD, Capacity.class))
        .thenReturn(Capacity.Builder.newInstance().build());
  ...
  public void whenOldVehicleIsNotNullAndSolutionComplete_itShouldReturnHalfOfFixedCostsOfNewVehicle() {
    ...
    // (1.*absFix + 0.*relFix) * completeness * weight = (1.*(100.-50.) + 0.*(50.-0.)) * 1. * 1. =
    // 50.
    assertEquals(50., absFixedCosts.getCosts(context) + relFixedCosts.getCosts(context), 0.01);
  }
  ...
  public void whenOldVehicleIsNotNullAndCurrentLoadIs25AndSolutionIs50PercentCompleteAndWeightIs05_itShouldReturnCorrectVal() {
    ...
    when(route.getVehicle()).thenReturn(small);
    when(stateGetter.getRouteState(route, InternalStates.MAXLOAD, Capacity.class))
        .thenReturn(Capacity.Builder.newInstance().addDimension(0, 25).build());
  ...
  public void whenOldVehicleIsNotNullAndCurrentLoadIs25AndSolutionIs50PercentCompleteAndWeightIs05WithMultipleCapDims_itShouldReturnCorrectVal() {
    ...
    when(job.getSize())
        .thenReturn(Capacity.Builder.newInstance().addDimension(0, 50).addDimension(1, 0).build());

    VehicleType oType = VehicleTypeImpl.Builder.newInstance("otype").addCapacityDimension(0, 50)
        .addCapacityDimension(1, 100).setFixedCost(50.0).build();
    when(small.getType()).thenReturn(oType);

    VehicleType type = VehicleTypeImpl.Builder.newInstance("type").addCapacityDimension(0, 100)
        .addCapacityDimension(1, 400).setFixedCost(100.0).build();
    ...
    // (0.5*absFix + 0.5*relFix) * 0.5 * 0.= (0.5*(100-50)+0.5*(75-25))*0.5*0.5 = 12.5
    /*
     * (0.5*(100-50)+0.5*( relFixNew - relFixOld = (75/100+100/400)/2.*100 -
     * ((25/50+100/100)/2.*50.) = )*0.5*0.5 = (0.5*(100-50)+0.5*((75/100+100/400)/2.*100 -
     * ((25/50+100/100)/2.*50.)))*0.5*0.5 = (0.5*(100-50)+0.5*12.5)*0.5*0.5 = 7.8125
     */
  ...
  public void whenOldVehicleIsMoreExpensive2() {
    ...
    when(stateGetter.getRouteState(route, InternalStates.MAXLOAD, Capacity.class)).thenReturn(
        Capacity.Builder.newInstance().addDimension(0, 25).addDimension(1, 100).build());
    /*
     * job = 50 abs = (50 - 100) * 0.1 * 0.1 * 1.0 = -0.5 rel = ( (75/50+100/100)/2 * 50 - (25/100 +
     * 100/400)/2 * 100) * 0.9 * 0.1 = 3.375 c = -0.5 + 3.375 = 2.875
     * 
     */
    JobInsertionContext context = new JobInsertionContext(route, job, medium, null, 0d);
  ...
  public void whenOldVehicleIsNotNullAndCurrentLoadIs25AndSolutionIs75PercentCompleteAndWeightIs05WithMultipleCapDims_itShouldReturnCorrectVal() {
    ...
    assertEquals(15.234375, absFixedCosts.getCosts(context) + relFixedCosts.getCosts(context),
        0.01);
  }
