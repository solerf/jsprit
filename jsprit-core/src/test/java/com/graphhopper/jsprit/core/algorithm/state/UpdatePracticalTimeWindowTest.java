...
public class UpdatePracticalTimeWindowTest {
  ...
  public void doBefore() {
    ...
    Pickup pickup = Pickup.Builder.newInstance("pick").setLocation(Location.newInstance("0,20"))
        .setTimeWindow(TimeWindow.newInstance(0, 30)).build();
    Delivery delivery =
        Delivery.Builder.newInstance("del").setLocation(Location.newInstance("20,20"))
            .setTimeWindow(TimeWindow.newInstance(10, 40)).build();
    Pickup pickup2 = Pickup.Builder.newInstance("pick2").setLocation(Location.newInstance("20,0"))
        .setTimeWindow(TimeWindow.newInstance(20, 50)).build();

    Vehicle vehicle = VehicleImpl.Builder.newInstance("v")
        .setStartLocation(Location.newInstance("0,0")).setType(mock(VehicleType.class)).build();
    ...
    route = VehicleRoute.Builder.newInstance(vehicle, mock(Driver.class))
        .setJobActivityFactory(new JobActivityFactory() {
          @Override
  ...
  public void whenVehicleRouteHasPickupAndDeliveryAndPickup_latestStartTimeOfAct3MustBeCorrect() {
    ...
    assertEquals(50., stateManager.getActivityState(route.getActivities().get(2),
  ...
  public void whenVehicleRouteHasPickupAndDeliveryAndPickup_latestStartTimeOfAct2MustBeCorrect() {
    ...
    assertEquals(30., stateManager.getActivityState(route.getActivities().get(1),
        InternalStates.LATEST_OPERATION_START_TIME, Double.class), 0.01);
  }
  ...
  public void whenVehicleRouteHasPickupAndDeliveryAndPickup_latestStartTimeOfAct1MustBeCorrect() {
    ...
    assertEquals(10., stateManager.getActivityState(route.getActivities().get(0),
        InternalStates.LATEST_OPERATION_START_TIME, Double.class), 0.01);
  }
