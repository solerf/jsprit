...
public class AlgorithmEventsRecorder
  ...
  private void addJob(Job job) {
    if (job instanceof Service) {
      ...
      if (renderShipments) {
        Edge e =
            graph.addEdge("shipment_" + fromNodeId + "_" + toNodeId, fromNodeId, toNodeId, true);
  ...
  private void insertNode(String nodeId, int insertionIndex, InsertionData data,
      VehicleRoute route) {
    // VehicleRoute copied = VehicleRoute.copyOf(route);
    ...
    if (isFirst(insertionIndex)) {
      ...
      TourActivity.JobActivity jobActivity =
          (TourActivity.JobActivity) route.getActivities().get(insertionIndex - 1);
    ...
    if (isLast(insertionIndex, route)) {
      ...
      TourActivity.JobActivity jobActivity =
          (TourActivity.JobActivity) route.getActivities().get(insertionIndex);
  ...
  private void handleVehicleSwitch(InsertionData data, VehicleRoute route) {
    ...
    if (vehicleSwitch && !route.getActivities().isEmpty()) {
      ...
      String lastAct =
          ((TourActivity.JobActivity) route.getActivities().get(route.getActivities().size() - 1))
              .getJob().getId();
