...
public class Plotter {
  ...
  private static class MyActivityRenderer extends XYLineAndShapeRenderer {
    ...
    MyActivityRenderer(XYSeriesCollection seriesCollection, Map<XYDataItem, Activity> activities,
  ...
  private BufferedImage plot(VehicleRoutingProblem vrp, final Collection<VehicleRoute> routes,
      String pngFile, String title) {
  ...
  private LegendTitle createLegend(final Collection<VehicleRoute> routes,
      final XYSeriesCollection shipments, final XYPlot plot) {
  ...
  private MyActivityRenderer getProblemRenderer(final XYSeriesCollection problem) {
    MyActivityRenderer problemRenderer =
        new MyActivityRenderer(problem, activitiesByDataItem, firstActivities);
  ...
  private XYSeriesCollection makeSolutionSeries(VehicleRoutingProblem vrp,
  ...
  private XYSeriesCollection makeShipmentSeries(Collection<Job> jobs) {
    ...
    for (Job job : jobs) {
      ...
      for (com.graphhopper.jsprit.core.problem.job.Activity act : job.getActivities()) {
        ...
        shipmentSeries.add(actCoordinate.getX() * scalingFactor,
            actCoordinate.getY() * scalingFactor);
      }
  ...
  private void addJob(XYSeries activities, Job job) {
    for (com.graphhopper.jsprit.core.problem.job.Activity act : job.getActivities()) {
      XYDataItem dataItem =
          new XYDataItem(getCoordinate(act.getLocation().getCoordinate()).getX() * scalingFactor,
              getCoordinate(act.getLocation().getCoordinate()).getY() * scalingFactor);
  ...
  private void retrieveActivities(VehicleRoutingProblem vrp) throws NoLocationFoundException {
    ...
    for (Vehicle v : vrp.getVehicles()) {
      ...
      XYDataItem item = new XYDataItem(startCoordinate.getX() * scalingFactor,
          startCoordinate.getY() * scalingFactor);
      ...
      if (!v.getStartLocation().getId().equals(v.getEndLocation().getId())) {
        ...
        if (endCoordinate == null)
          throw new NoLocationFoundException();
        XYDataItem end_item = new XYDataItem(endCoordinate.getX() * scalingFactor,
            endCoordinate.getY() * scalingFactor);
