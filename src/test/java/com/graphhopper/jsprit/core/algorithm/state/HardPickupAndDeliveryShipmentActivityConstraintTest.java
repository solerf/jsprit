...
public class HardPickupAndDeliveryShipmentActivityConstraintTest {
  ...
  public void doBefore() {
    ...
    shipment = Shipment.Builder.newInstance("shipment")
    ...
    vrp = VehicleRoutingProblem.Builder.newInstance().addJob(s1).addJob(s2).addJob(shipment)
  ...
  public void whenPickupActivityIsInsertedAndLoadIsNotSufficient_returnNOT_FullFilled() {
    ...
    stateManager.putInternalTypedActivityState(pickupService, InternalStates.LOAD,
        Capacity.Builder.newInstance().addDimension(0, 2).build());
    // when(stateManager.getActivityState(pickupService,
    // StateFactory.LOAD)).thenReturn(StateFactory.createState(2.0));
  ...
  public void whenDeliveryActivityIsInsertedAndLoadIsSufficient_returnFullFilled() {
    ...
    stateManager.putInternalTypedActivityState(pickupService, InternalStates.LOAD,
        Capacity.Builder.newInstance().addDimension(0, 1).build());
    // stateManager.putInternalActivityState(pickupService, StateFactory.LOAD,
    // StateFactory.createState(1));
    assertEquals(ConstraintsStatus.FULFILLED,
