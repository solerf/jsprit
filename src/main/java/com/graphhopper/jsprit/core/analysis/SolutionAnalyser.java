...
public class SolutionAnalyser {
  ...
  private static class BackhaulAndShipmentUpdater implements StateUpdater, ActivityVisitor {
    ...
    private BackhaulAndShipmentUpdater(StateId backhaul_id, StateId shipment_id,
    ...
    public void visit(TourActivity activity) {
      ...
      if (activity instanceof DeliverService && pickupOccured) {
        ...
        if (activity instanceof PickupService || activity instanceof ServiceActivity
  ...
  private static class SumUpActivityTimes implements StateUpdater, ActivityVisitor {
    ...
    private SumUpActivityTimes(StateId waiting_time_id, StateId transport_time_id,
        StateId service_time_id, StateId too_late_id, StateManager stateManager,
    ...
    public void visit(TourActivity activity) {
      ...
      if (activityPolicy.equals(ActivityTimeTracker.ActivityPolicy.AS_SOON_AS_TIME_WINDOW_OPENS)) {
        waitAtAct = Math.max(0,
            activity.getTheoreticalEarliestOperationStartTime() - activity.getArrTime());
        tooLate =
      ...
      // service time
      sum_service_time += activityCosts.getActivityDuration(activity, activity.getArrTime(),
    ...
    public void finish() {
      ...
      sum_too_late += Math.max(0,
          route.getEnd().getArrTime() - route.getEnd().getTheoreticalLatestOperationStartTime());
  ...
  private static class LastTransportUpdater implements StateUpdater, ActivityVisitor {
    ...
    private LastTransportUpdater(StateManager stateManager,
        VehicleRoutingTransportCosts transportCost, TransportDistance distanceCalculator,
        StateId last_distance_id, StateId last_time_id, StateId last_cost_id) {
      this.stateManager = stateManager;
    ...
    private double transportCost(TourActivity activity) {
      return transportCost.getTransportCost(prevAct.getLocation(), activity.getLocation(),
          prevActDeparture, route.getDriver(), route.getVehicle());
    }
    ...
    private double distance(TourActivity activity) {
      return distanceCalculator.getDistance(prevAct.getLocation(), activity.getLocation(),
          prevActDeparture, route.getVehicle());
    }
  ...
  private static class DistanceUpdater implements StateUpdater, ActivityVisitor {
    ...
    private DistanceUpdater(StateId distanceId, StateManager stateManager,
    ...
    @Override
    public void visit(TourActivity activity) {
      double distance = distanceCalculator.getDistance(prevAct.getLocation(),
          activity.getLocation(), prevAct.getEndTime(), route.getVehicle());
    ...
    public void finish() {
      double distance = distanceCalculator.getDistance(prevAct.getLocation(),
          route.getEnd().getLocation(), prevAct.getEndTime(), route.getVehicle());
      sumDistance += distance;
   ...
   */
  public SolutionAnalyser(VehicleRoutingProblem vrp, VehicleRoutingProblemSolution solution,
      TransportDistance distanceCalculator) {
    this.vrp = vrp;
    ...
    this.solutionCostCalculator =
        new VariablePlusFixedSolutionCostCalculatorFactory(stateManager).createCalculator();
    refreshStates();
  }

  public SolutionAnalyser(VehicleRoutingProblem vrp, VehicleRoutingProblemSolution solution,
      SolutionCostCalculator solutionCostCalculator, TransportDistance distanceCalculator) {
    this.vrp = vrp;
    this.solution = solution;
    this.distanceCalculator = distanceCalculator;
  ...
  private void initialise() {
    ...
    stateManager.addStateUpdater(new SumUpActivityTimes(waitingTimeId, transportTimeId,
        serviceTimeId, tooLateId, stateManager, activityPolicy, vrp.getActivityCosts()));
    stateManager.addStateUpdater(new DistanceUpdater(distanceId, stateManager, distanceCalculator));
    stateManager
        .addStateUpdater(new BackhaulAndShipmentUpdater(backhaulId, shipmentId, stateManager));
    ...
    stateManager.addStateUpdater(new LastTransportUpdater(stateManager, vrp.getTransportCosts(),
        distanceCalculator, lastTransportDistanceId, lastTransportTimeId, lastTransportCostId));
  }
   ...
   *         atBeginning of the specified route. If act is End, it returns the load atEnd of
   *         specified route. Returns null if no load can be found.
   */
  ...
  public Integer getNumberOfPickups(VehicleRoute route) {
    if (route == null)
      throw new IllegalArgumentException("route is missing.");
    return stateManager.getRouteState(route, stateManager.createStateId(PICKUP_COUNT),
        Integer.class);
  }
  ...
  public Integer getNumberOfDeliveries(VehicleRoute route) {
    if (route == null)
      throw new IllegalArgumentException("route is missing.");
    return stateManager.getRouteState(route, stateManager.createStateId(DELIVERY_COUNT),
        Integer.class);
  }
  ...
  public Capacity getLoadPickedUp(VehicleRoute route) {
    if (route == null)
      throw new IllegalArgumentException("route is missing.");
    return stateManager.getRouteState(route, stateManager.createStateId(LOAD_PICKED),
        Capacity.class);
  }
  ...
  public Capacity getLoadDelivered(VehicleRoute route) {
    if (route == null)
      throw new IllegalArgumentException("route is missing.");
    return stateManager.getRouteState(route, stateManager.createStateId(LOAD_DELIVERED),
        Capacity.class);
  }
  ...
  public Capacity getCapacityViolation(VehicleRoute route) {
    ...
    return Capacity.max(Capacity.Builder.newInstance().build(),
        Capacity.subtract(maxLoad, route.getVehicle().getType().getCapacityDimensions()));
  }
  ...
  public Capacity getCapacityViolationAtBeginning(VehicleRoute route) {
    ...
    return Capacity.max(Capacity.Builder.newInstance().build(),
        Capacity.subtract(atBeginning, route.getVehicle().getType().getCapacityDimensions()));
  }
   ...
   * @return true if skill constraint is violated, i.e. if vehicle does not have the required skills
   *         to conduct all activities on the specified route. Returns null if route is null or
   *         skill state cannot be found.
   */
   ...
   *         otherwise. Returns null if specified route or activity is null or if route does not
   *         contain specified activity or if skill state connot be found. If specified activity is
   *         Start or End, it returns false.
   */
  ...
  /**
   * Returns true if backhaul constraint is violated (false otherwise). Backhaul constraint is
   * violated if either a pickupService, serviceActivity (which is basically modeled as
   * pickupService) or a pickupShipment occur before a deliverService activity or - to put it in
   * other words - if a depot bounded delivery occurs after a pickup, thus the backhaul ensures
   * depot bounded delivery activities first.
   *
   * @param route to check backhaul constraint
   * @return true if backhaul constraint for specified route is violated. returns null if route is
   *         null or no backhaul state can be found. In latter case try routeChanged(route).
   */
   ...
   * @return true if backhaul constraint is violated, false otherwise. Null if either specified
   *         route or activity is null. Null if specified route does not contain specified activity.
   */
  public Boolean hasBackhaulConstraintViolationAtActivity(TourActivity activity,
      VehicleRoute route) {
    if (route == null)
      throw new IllegalArgumentException("route is missing.");
    if (activity == null)
  ...
  /**
   * Returns true if shipment constraint is violated, i.e. if activity is deliverShipment but no
   * pickupShipment can be found before OR activity is pickupShipment and no deliverShipment can be
   * found afterwards.
   *
   * @param activity to check the shipment constraint
   * @param route that must contain specified activity
   * @return true if shipment constraint is violated, false otherwise. If activity is either Start
   *         or End, it returns false. Returns null if either specified activity or route is null or
   *         route does not containt activity.
   */
  public Boolean hasShipmentConstraintViolationAtActivity(TourActivity activity,
      VehicleRoute route) {
    if (route == null)
      throw new IllegalArgumentException("route is missing.");
    if (activity == null)
   ...
   * @return total transport time of specified route. Returns null if no time value exists for the
   *         specified route.
   */
   ...
   * @return total service time of specified route. Returns null if no time value exists for
   *         specified route.
   */
   ...
   * @param route where the activity should be part of
   * @return transport time at the activity, i.e. the total time spent driving since the start of
   *         the route to the specified activity.
   */
  ...
  public Integer getNumberOfPickupsAtBeginning(VehicleRoute route) {
    if (route == null)
      throw new IllegalArgumentException("route is missing.");
    return stateManager.getRouteState(route, stateManager.createStateId(PICKUP_COUNT_AT_BEGINNING),
        Integer.class);
  }
  ...
  public Integer getNumberOfDeliveriesAtEnd(VehicleRoute route) {
    if (route == null)
      throw new IllegalArgumentException("route is missing.");
    return stateManager.getRouteState(route, stateManager.createStateId(DELIVERY_COUNT_AT_END),
        Integer.class);
  }
