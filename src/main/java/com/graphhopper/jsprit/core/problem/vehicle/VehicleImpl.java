...
public class VehicleImpl extends AbstractVehicle {
   ...
   * <p>
   * <p>
   * By default, earliestDepartureTime is 0.0, latestDepartureTime is Double.MAX_VALUE, it returns
   * to the depot and its {@link VehicleType} is the DefaultType with typeId equal to 'default' and
   * a capacity of 0.
   *
   * @author stefan
   */
  public static class Builder {
     ...
     * <p>
     * <p>
     * The user data is a black box for the framework, it only stores it, but never interacts with
     * it in any way.
     * </p>
     *
     ...
     * <p>
     * <p>
     * If returnToDepot is true, the vehicle must return to specified end-location. If you omit
    ...
    public Builder setStartLocation(Location startLocation) {
      if (startLocation == null)
        throw new IllegalArgumentException(
            "Start location of vehicle " + id + " must not be null.");
    ...
    public Builder setEarliestStart(double earliest_startTime) {
      if (earliest_startTime < 0)
        throw new IllegalArgumentException(
            "The earliest start time of vehicle " + id + " must not be negative.");
    ...
    public Builder setLatestArrival(double latest_arrTime) {
      if (latest_arrTime < 0)
        throw new IllegalArgumentException(
            "The latest arrival time of vehicle " + id + " must not be negative.");
     ...
     * <p>
     * <p>
     * if startLocationId || locationId is null (=> startLocationCoordinate || locationCoordinate
     ...
     * <p>
     * if endLocationId is null and endLocationCoordinate is set then
     * endLocationId=endLocationCoordinate.toString()
     * <p>
     * if endLocationId==null AND endLocationCoordinate==null then endLocationId=startLocationId AND
     * endLocationCoord=startLocationCoord Thus endLocationId can never be null even returnToDepot
     * is false.
     *
     * @return vehicle
     * @throws IllegalArgumentException if both locationId and locationCoord is not set or
     *         (endLocationCoord!=null AND returnToDepot=false) or (endLocationId!=null AND
     *         returnToDepot=false)
     */
    public VehicleImpl build() {
      if (latestArrival < earliestStart)
        throw new IllegalArgumentException("The latest arrival time of vehicle " + id
            + " must not be smaller than its start time.");
      if (startLocation != null && endLocation != null) {
        if (!startLocation.getId().equals(endLocation.getId()) && !returnToDepot)
          throw new IllegalArgumentException(
              "You specified both the end location and that the vehicle " + id
                  + " does not need to return to its end location. This must not be. "
      ...
      if (startLocation == null && endLocation == null)
        throw new IllegalArgumentException(
            "Every vehicle requires a start location, but vehicle " + id + " does not have one.");
    ...
    /**
     * Returns new instance of vehicle builder and initializes every attribute with a attributes of
     * baseVehicle
     *
  ...
  private VehicleImpl(Builder builder) {
    ...
    // setVehicleIdentifier(new
    // VehicleTypeKey(type.getTypeId(),startLocation.getId(),endLocation.getId(),earliestDeparture,latestArrival,skills));
    setVehicleIdentifier(new VehicleTypeKey(type.getTypeId(), startLocation.getId(),
        endLocation.getId(), earliestDeparture, latestArrival, skills, returnToDepot));
  }
  ...
  public String toString() {
    return "[id=" + id + "]" + "[type=" + type + "]" + "[startLocation=" + startLocation + "]"
        + "[endLocation=" + endLocation + "]" + "[isReturnToDepot=" + isReturnToDepot() + "]"
        + "[skills=" + skills + "]";
  }
