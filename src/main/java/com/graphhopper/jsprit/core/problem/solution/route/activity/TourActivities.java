...
public class TourActivities {
  ...
  /**
   * Removes job AND belonging activity from tour. Note that if job is a Service, it is assumed that
   * there is only one belonging activity, thus, it stops trying to remove activities once the first
   * is found. If you want to always look for belonging activities to be removed in the entire
   * route, use removeJob(Job job, boolean enforceEntireRoute)
   *
  ...
  /**
   * Removes activity from this activity sequence. Removes its corresponding job as well, if there
   * are no other activities related to this job.
   *
  ...
  /**
   * Inserts the specified activity add the specified insertionIndex. Shifts the element currently
  ...
  public void addActivity(int insertionIndex, TourActivity act) {
    ...
    /*
     * if 1 --> between start and act(0) --> act(0) if 2 && 2 <= acts.size --> between act(0) and
     * act(1) --> act(1) if 2 && 2 > acts.size --> at actEnd ...
     *
     */
