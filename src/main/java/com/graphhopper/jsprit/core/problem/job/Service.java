...
public class Service extends AbstractJob {
  ...
  public static class Builder<T extends Service> {
     ...
     * <p>
     * <p>
     * It is understood as time that a service or its implied activity takes at the
     * service-location, for instance to unload goods.
     *
    ...
    public Builder<T> setServiceTime(double serviceTime) {
      if (serviceTime < 0)
        throw new IllegalArgumentException(
            "The service time of a service must be greater than or equal to zero.");
    ...
    public T build() {
      ...
      activity = new Activity.Builder(location, Activity.Type.SERVICE).setServiceTime(serviceTime)
          .setTimeWindows(timeWindows.getTimeWindows()).build();
    ...
    public Builder<T> setMaxTimeInVehicle(double maxTimeInVehicle) {
      throw new UnsupportedOperationException(
          "The maximum time in vehicle is not yet supported for Pickups and Services (only for Deliveries and Shipments).");
      // if(maxTimeInVehicle < 0) throw new IllegalArgumentException("maxTimeInVehicle should be
      // positive");
  ...
  /**
   * Returns the time-window a service(-operation) is allowed to start. It is recommended to use
   * getTimeWindows() instead. If you still use this, it returns the first time window of
  ...
  public String toString() {
    return "[id=" + id + "][name=" + name + "][type=" + type + "][location=" + location
        + "][capacity=" + size + "][serviceTime=" + serviceTime + "][timeWindows=" + timeWindows
        + "]";
  }
