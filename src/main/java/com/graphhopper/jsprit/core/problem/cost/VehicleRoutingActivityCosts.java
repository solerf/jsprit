 ...
 * <p>
 * <p>
 * This calculates activity and leg-based costs. If you want to consider for example costs incurred
 * by missed time-windows, you can do it here.
 *
 * @author schroeder
 */
public interface VehicleRoutingActivityCosts {
   ...
   *
   * @param tourAct
   * @param arrivalTime is actually the arrival time at this tourActivity, which must not
   *        nessecarrily be the operation start time. If the theoretical earliest operation start
   *        time at this activity is later than actualStartTime, the driver must wait at this
   *        activity.
   * @param driver
   * @param vehicle if earliestStartTime > latestStartTime activity operations cannot be conducted
   *        within the given time-window.
   * @return
   */
  double getActivityCost(TourActivity tourAct, double arrivalTime, Driver driver, Vehicle vehicle);

  double getActivityDuration(TourActivity tourAct, double arrivalTime, Driver driver,
      Vehicle vehicle);
