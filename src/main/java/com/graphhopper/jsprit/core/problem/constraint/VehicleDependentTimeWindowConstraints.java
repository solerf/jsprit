...
public class VehicleDependentTimeWindowConstraints implements HardActivityConstraint {
  ...
  public VehicleDependentTimeWindowConstraints(RouteAndActivityStateGetter states,
  ...
  public ConstraintsStatus fulfilled(JobInsertionContext iFacts, TourActivity prevAct,
    ...
    if (nextAct instanceof End) {
      ...
      latestArrTimeAtNextAct = states.getActivityState(nextAct, iFacts.getNewVehicle(),
          InternalStates.LATEST_OPERATION_START_TIME, Double.class);
      if (latestArrTimeAtNextAct == null) {// otherwise set it to
                                           // theoretical_latest_operation_startTime
    ...
    /*
     * if latest arrival of vehicle (at its end) is smaller than earliest operation start times of
     * activities, then vehicle can never conduct activities.
     *
     * |--- vehicle's operation time ---| |--- prevAct or newAct or nextAct ---|
     */
    double newAct_theoreticalEarliestOperationStartTime =
    ...
    /*
     * if the latest operation start-time of new activity is smaller than the earliest start of
     * prev. activity, then
     *
     * |--- prevAct ---| |--- newAct ---|
     */
    if (newAct.getTheoreticalLatestOperationStartTime() < prevAct
        .getTheoreticalEarliestOperationStartTime()) {
      return ConstraintsStatus.NOT_FULFILLED_BREAK;
    }

    /*
     * |--- prevAct ---| |- earliest arrival of vehicle |--- nextAct ---|
     */
    double arrTimeAtNextOnDirectRouteWithNewVehicle =
        prevActDepTime + routingCosts.getTransportTime(prevAct.getLocation(), nextActLocation,
     ...
     * |--- newAct ---| |--- nextAct ---|
     */
    if (newAct.getTheoreticalEarliestOperationStartTime() > nextAct
        .getTheoreticalLatestOperationStartTime()) {
      return ConstraintsStatus.NOT_FULFILLED;
    }
    // log.info("check insertion of " + newAct + " between " + prevAct + " and " + nextAct + ".
    // prevActDepTime=" + prevActDepTime);
    ...
    double endTimeAtNewAct =
        Math.max(arrTimeAtNewAct, newAct.getTheoreticalEarliestOperationStartTime())
            + activityCosts.getActivityDuration(newAct, arrTimeAtNewAct, iFacts.getNewDriver(),
                iFacts.getNewVehicle());
    double latestArrTimeAtNewAct = Math.min(newAct.getTheoreticalLatestOperationStartTime(),
        latestArrTimeAtNextAct
            - routingCosts.getBackwardTransportTime(newAct.getLocation(), nextActLocation,
                latestArrTimeAtNextAct, iFacts.getNewDriver(), iFacts.getNewVehicle())
            - activityCosts.getActivityDuration(newAct, arrTimeAtNewAct, iFacts.getNewDriver(),
                iFacts.getNewVehicle()));

    /*
     * |--- prevAct ---| |--- vehicle's arrival @newAct latest arrival of vehicle @newAct ---|
     */
    ...
    double arrTimeAtNextAct = endTimeAtNewAct + routingCosts.getTransportTime(newAct.getLocation(),
        nextActLocation, endTimeAtNewAct, iFacts.getNewDriver(), iFacts.getNewVehicle());

    /*
     * |--- newAct ---| |--- vehicle's arrival @nextAct latest arrival of vehicle @nextAct ---|
     */
