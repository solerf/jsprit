...
public class UpdateMaxTimeInVehicle implements StateUpdater, ActivityVisitor {
  ...
  private final Map<Integer, Map<TourActivity, Double>> slackTimesPerVehicle = new HashMap<>();

  private final Map<Integer, Map<TourActivity, Double>> actStartTimesPerVehicle = new HashMap<>();
  ...
  private UpdateVehicleDependentPracticalTimeWindows.VehiclesToUpdate vehiclesToUpdate =
  ...
  public UpdateMaxTimeInVehicle(StateManager stateManager, StateId slackTimeId,
      TransportTime transportTime, VehicleRoutingActivityCosts activityCosts, StateId openJobsId) {
  ...
  public void setVehiclesToUpdate(
      UpdateVehicleDependentPracticalTimeWindows.VehiclesToUpdate vehiclesToUpdate) {
  ...
  public void visit(TourActivity activity) {
    ...
    for (Vehicle v : vehicles) {
      ...
      double activityArrival =
          prevActEndTimes[v.getVehicleTypeIdentifier().getIndex()] + transportTime.getTransportTime(
              prevActLocation, activity.getLocation(), prevActEndTime, route.getDriver(), v);
      double activityStart =
          Math.max(activityArrival, activity.getTheoreticalEarliestOperationStartTime());
      memorizeActStart(activity, v, activityStart);
      double activityEnd = activityStart
          + activityCosts.getActivityDuration(activity, activityArrival, route.getDriver(), v);
  ...
  private void memorizeActStart(TourActivity activity, Vehicle v, double activityStart) {
    actStartTimesPerVehicle.get(v.getVehicleTypeIdentifier().getIndex()).put(activity,
        activityStart);
  }
  ...
  public void finish() {
    for (Vehicle v : vehicles) {
      ...
      if (!v.isReturnToDepot())
        ...
        routeEnd = prevActEndTimes[vehicleIndex]
            + transportTime.getTransportTime(prevActLocations[vehicleIndex], v.getEndLocation(),
                prevActEndTimes[vehicleIndex], route.getDriver(), v);
