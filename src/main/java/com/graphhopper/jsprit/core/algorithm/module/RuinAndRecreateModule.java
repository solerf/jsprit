...
public class RuinAndRecreateModule implements SearchStrategyModule {
  ...
  /**
   * To make overall results reproducible, make sure this class is provided with the "global" random
   * number generator.
   *
   * @param random
   */
   ...
   */
  public void setProportionOfUnassignedJobsToBeReinserted(
      double proportionOfUnassignedJobsToBeReinserted) {
    this.proportionOfUnassignedJobsToBeReinserted = proportionOfUnassignedJobsToBeReinserted;
  }

  @Override
  public VehicleRoutingProblemSolution runAndGetSolution(
      VehicleRoutingProblemSolution previousVrpSolution) {
    ...
    if (previousVrpSolution.getUnassignedJobs().size() < minUnassignedJobsToBeReinserted) {
      ...
      int noUnassignedToBeInserted = Math.max(minUnassignedJobsToBeReinserted,
          (int) (previousVrpSolution.getUnassignedJobs().size()
    ...
    Collection<Job> unassignedJobs =
        insertion.insertJobs(previousVrpSolution.getRoutes(), ruinedJobSet);
