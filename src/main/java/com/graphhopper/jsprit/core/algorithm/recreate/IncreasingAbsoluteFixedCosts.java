...
public final class IncreasingAbsoluteFixedCosts extends SolutionCompletenessRatio
  ...
  public double getCosts(JobInsertionContext insertionContext) {
    ...
    if (currentRoute.getVehicle() != null
        && !(currentRoute.getVehicle() instanceof VehicleImpl.NoVehicle)) {
      currentFix = currentRoute.getVehicle().getType().getVehicleCostParams().fix;
    }
    double increasingAbsoluteFixedCosts = solutionCompletenessRatio
        * (insertionContext.getNewVehicle().getType().getVehicleCostParams().fix - currentFix);
