 ...
 * <p>
 * Technical report 7.11.2016 A Fresh Ruin & Recreate Implementation for the Capacitated Vehicle
 * Routing Problem Jan Christiaens, Greet Vanden Berghe KU Leuven, Department of Computer Science,
 * CODeS & iMinds-ITEC Gebr. De Smetstraat 1, 9000 Gent, Belgium, jan.christiaens@cs.kuleuven.be,
...
public final class RuinString extends AbstractRuinStrategy {
  ...
  public Collection<Job> ruinRoutes(Collection<VehicleRoute> vehicleRoutes) {
    ...
    Iterator<Job> neighborhoodIterator =
        jobNeighborhoods.getNearestNeighborsIterator(kMax * lMax, prevJob);
  ...
  private void ruinRouteWithSplitStringRuin(VehicleRoute seedRoute, Job prevJob,
      Set<Job> unassignedJobs) {
    ...
    int preservedSubstringLength =
        StringUtil.determineSubstringLength(stringLength, noActivities, random);
    ...
    List<Integer> stringBounds =
        StringUtil.getLowerBoundsOfAllStrings(totalStringLength, seedIndex, noActivities);
  ...
  private void ruinRouteWithStringRuin(VehicleRoute seedRoute, Job prevJob,
      Set<Job> unassignedJobs) {
    ...
    List<Integer> stringBounds =
        StringUtil.getLowerBoundsOfAllStrings(stringLength, seedIndex, noActivities);
    if (stringBounds.isEmpty())
      return;
    int lowerBound = RandomUtils.nextItem(stringBounds, random);
    List<Job> jobs2Remove = new ArrayList<>();
