 ...
 * <p>
 * <p>
 * Some condition, rules or constraints are stateful. This StateManager manages these states, i.e.
 * it offers methods to add, store and retrieve states based on the problem, vehicle-routes and
 * tour-activities.
 *
 * @author schroeder
 */
public class StateManager implements RouteAndActivityStateGetter, IterationStartsListener,
    RuinListener, InsertionStartsListener, JobInsertedListener, InsertionEndsListener {

  private final RouteActivityVisitor routeActivityVisitor = new RouteActivityVisitor();

  private final ReverseRouteActivityVisitor revRouteActivityVisitor =
      new ReverseRouteActivityVisitor();
   ...
   * </p>
   * <p>
   * If the specified is equal to a name that is already used internally, it throws an
   * IllegalStateException
   * </p>
   *
   * @param name the specified name of the state
   * @return the stateId with which a state can be identified, no matter if it is a problem, route
   *         or activity state.
  ...
  public StateId createStateId(String name) {
    ...
    if (stateIndexCounter >= activityStates[0].length) {
      ...
      vehicleDependentActivityStates =
          new Object[nuActivities][nuVehicleTypeKeys][stateIndexCounter + 1];
      routeStatesArr = new Object[vrp.getVehicles().size() + 2][stateIndexCounter + 1];
      vehicleDependentRouteStatesArr =
          new Object[vrp.getVehicles().size() + 2][nuVehicleTypeKeys][stateIndexCounter + 1];
  ...
  public StateManager(VehicleRoutingProblem vehicleRoutingProblem) {
    ...
    vehicleDependentActivityStates =
        new Object[nuActivities][nuVehicleTypeKeys][initialStateArrayLength];
    ...
    // vehicleDependentRouteStatesArr = new Object[vrp.getVehicles().size() +
    // 2][nuVehicleTypeKeys][initialStateArrayLength];
    // }
    // else {
  ...
  /**
   * Associates the specified state to the stateId. If there already exists a state value for the
   * stateId, this old value is replaced by the new value.
   *
  ...
  /**
   * Returns mapped state value that is associated to the specified stateId, or null if no value is
   * associated to the specified stateId.
   *
   * @param stateId the stateId which is the associated key to the problem state
   * @param type the type class of the state value
   * @param <T> the type
   * @return the state value that is associated to the specified stateId or null if no value is
   *         associated
   */
  ...
  /**
   * Returns associated state for the specified activity and stateId, or it returns null if no value
   * is associated.
   * <p>
   * If type class is not equal to the associated type class of the requested state value, it throws
   ...
   * @param <T> the type
   * @return the state value that is associated to the specified activity and stateId, or null if no
   *         value is associated.
   * @throws java.lang.ClassCastException if type class is not equal to the associated type class of
   *         the requested state value
   * @throws java.lang.IllegalStateException if <code>act.getIndex()==0</code> since this suggests
   *         that act has no index at all
   */
  @Override
  public <T> T getActivityState(TourActivity act, StateId stateId, Class<T> type) {
    ...
    try {
      ...
      throw getClassCastException(e, stateId, type.toString(),
          activityStates[act.getIndex()][stateId.getIndex()].getClass().toString());
    }
    return state;
  }
  ...
  public boolean hasActivityState(TourActivity act, Vehicle vehicle, StateId stateId) {
    if (act.getIndex() == 0)
      throw new IllegalStateException("activity index is 0. this should not be.");
    return vehicleDependentActivityStates[act.getIndex()][vehicle.getVehicleTypeIdentifier()
        .getIndex()][stateId.getIndex()] != null;
  }

  /**
   * Returns the associated state value to the specified activity, vehicle and stateId, or null if
   * no state value is associated.
   * <p>
   * If type class is not equal to the associated type class of the requested state value, it throws
   * a ClassCastException.
   * </p>
   ...
   * @return the associated state value to the specified activity, vehicle and stateId, or null if
   *         no state value is associated.
   * @throws java.lang.ClassCastException if type class is not equal to the associated type class of
   *         the requested state value
   * @throws java.lang.IllegalStateException if <code>act.getIndex()==0</code> since this suggests
  ...
  public <T> T getActivityState(TourActivity act, Vehicle vehicle, StateId stateId, Class<T> type) {
    if (act.getIndex() == 0)
      throw new IllegalStateException("activity index is 0. this should not be.");
    if (act.getIndex() < 0)
      return null; // act.getIndex() < 0 indicates that act is either Start (-1) or End (-2)
    T state;
    try {
      state = type.cast(vehicleDependentActivityStates[act.getIndex()][vehicle
          .getVehicleTypeIdentifier().getIndex()][stateId.getIndex()]);
    } catch (ClassCastException e) {
      Object state_class = vehicleDependentActivityStates[act.getIndex()][vehicle
          .getVehicleTypeIdentifier().getIndex()][stateId.getIndex()];
  ...
  private ClassCastException getClassCastException(ClassCastException e, StateId stateId,
      String requestedTypeClass, String memorizedTypeClass) {
    return new ClassCastException(e + "\n" + "state with stateId '" + stateId.toString()
        + "' is of " + memorizedTypeClass + ". cannot cast it to " + requestedTypeClass + ".");
  }

  /**
   * Returns the route state that is associated to the route and stateId, or null if no state is
   * associated.
   * <p>
   * If type class is not equal to the associated type class of the requested state value, it throws
   * a ClassCastException.
   ...
   * @param type the class of the associated state value
   * @param <T> the type of the class
   * @return the route state that is associated to the route and stateId, or null if no state is
   *         associated.
   * @throws java.lang.ClassCastException if type class is not equal to the associated type class of
   *         the requested state value
   * @throws java.lang.IllegalStateException if <code>!route.isEmpty()</code> and
   *         <code>act(0).getIndex()==0</code> since this suggests that act has no index at all
   */
  @Override
  public <T> T getRouteState(VehicleRoute route, StateId stateId, Class<T> type) {
    ...
    if (isIndexedBased) {
      try {
        ...
        throw getClassCastException(e, stateId, type.toString(),
            routeStatesArr[route.getVehicle().getIndex()][stateId.getIndex()].getClass()
                .toString());
      }
    } else {
      try {
        ...
        throw getClassCastException(e, stateId, type.toString(),
            routeStateMap.get(route)[stateId.getIndex()].getClass().toString());
      }
    }
    return state;
  }
  ...
  /**
   * Returns true if a state is assigned to the specified route, vehicle and stateId. Otherwise it
   * returns false.
   *
  ...
  public boolean hasRouteState(VehicleRoute route, Vehicle vehicle, StateId stateId) {
    if (!vehicleDependentRouteStateMap.containsKey(route))
      return false;
    return vehicleDependentRouteStateMap.get(route)[vehicle.getVehicleTypeIdentifier()
        .getIndex()][stateId.getIndex()] != null;
    // return
    // vehicle_dependent_route_states[route.getActivities().get(0).getIndex()][vehicle.getVehicleTypeIdentifier().getIndex()][stateId.getIndex()]
    // != null;
  }
  ...
  public <T> T getRouteState(VehicleRoute route, Vehicle vehicle, StateId stateId, Class<T> type) {
    ...
    if (isIndexedBased) {
      try {
        state = type.cast(vehicleDependentRouteStatesArr[route.getVehicle().getIndex()][vehicle
            .getVehicleTypeIdentifier().getIndex()][stateId.getIndex()]);
      } catch (ClassCastException e) {
        throw getClassCastException(e, stateId, type.toString(),
            vehicleDependentRouteStatesArr[route.getVehicle().getIndex()][vehicle
                .getVehicleTypeIdentifier().getIndex()][stateId.getIndex()].getClass().toString());
      }
    } else {
      try {
        if (vehicleDependentRouteStateMap.containsKey(route)) {
          state = type.cast(vehicleDependentRouteStateMap.get(route)[vehicle
              .getVehicleTypeIdentifier().getIndex()][stateId.getIndex()]);
        }
      } catch (ClassCastException e) {
        throw getClassCastException(e, stateId, type.toString(),
            vehicleDependentRouteStateMap.get(route)[vehicle.getVehicleTypeIdentifier()
                .getIndex()][stateId.getIndex()].getClass().toString());
      }
    }
    return state;
  }
  ...
  /**
   * Associates the specified activity and stateId to the state value. If a state value is already
   * associated to the specified activity and stateId, it is replaced by the new state value.
   *
   * @param act the activity for which a state value is associated to
   ...
   * @throws java.lang.IllegalStateException if <code>act.getIndex() == 0</code> || stateId.getIndex
   *         < noInternalStates
   */
  ...
  /**
   * Associates the specified activity, vehicle and stateId to the state value. If a state value is
   * already associated to the specified activity and stateId, it is replaced by the new state
   * value.
   *
   * @param act the activity for which a state value is associated to
   * @param vehicle the vehicle for which a state value is associated to
  ...
  <T> void putInternalTypedActivityState(TourActivity act, Vehicle vehicle, StateId stateId,
      T state) {
    vehicleDependentActivityStates[act.getIndex()][vehicle.getVehicleTypeIdentifier()
        .getIndex()][stateId.getIndex()] = state;
  }

  /**
   * Associates the specified route, vehicle and stateId to the state value. If a state value is
   * already associated to the specified activity and stateId, it is replaced by the new state
   * value.
   *
   ...
   * @param state the state that is associated to the activity and stateId
   * @param <T> the type of the state
   * @throws java.lang.IllegalStateException if stateId is equal to a stateId that is already used
   *         internally.
   */
   ...
   * @param state the state that is associated to the activity and stateId
   * @param <T> the type of the state
   * @throws java.lang.IllegalStateException if <code>vehicle.getIndex() == 0</code> ||
   *         <code>stateId.getIndex() < noInternalStates</code>
   */
  ...
  <T> void putTypedInternalRouteState(VehicleRoute route, Vehicle vehicle, StateId stateId,
      T state) {
    // if (route.isEmpty()) return;
    if (isIndexedBased) {
      vehicleDependentRouteStatesArr[route.getVehicle().getIndex()][vehicle
          .getVehicleTypeIdentifier().getIndex()][stateId.getIndex()] = state;
    } else {
      if (!vehicleDependentRouteStateMap.containsKey(route)) {
   ...
   * <p>
   * <p>
   * Note that a state update occurs if route and/or activity states have changed, i.e. if jobs are
   * removed or inserted into a route. Thus here, it is assumed that a state updater is either of
   * type InsertionListener, RuinListener, ActivityVisitor, ReverseActivityVisitor, RouteVisitor,
   * ReverseRouteVisitor.
   * <p>
   * <p>
   * The following rule pertain for activity/route visitors:These visitors visits all
   * activities/route in a route subsequently in two cases. First, if insertionStart (after
   * ruinStrategies have removed activities from routes) and, second, if a job has been inserted and
   * thus if a route has changed.
   *
   ...
   * <p>
   * This visitor visits all activities in a route subsequently in two cases. First, if
   * insertionStart (after ruinStrategies have removed activities from routes) and, second, if a job
   * has been inserted and thus if a route has changed.
   *
   ...
   * <p>
   * This reverseVisitor visits all activities in a route subsequently (starting from the end of the
   * route) in two cases. First, if insertionStart (after ruinStrategies have removed activities
   * from routes) and, second, if a job has been inserted and thus if a route has changed.
   *
  ...
  @Override
  public void informJobInserted(Job job2insert, VehicleRoute inRoute, double additionalCosts,
  ...
  @Override
  public void informInsertionStarts(Collection<VehicleRoute> vehicleRoutes,
  ...
  public void updateLoadStates() {
    if (!updateLoad) {
      ...
      addActivityVisitor(
          new UpdateMaxCapacityUtilisationAtActivitiesByLookingBackwardInRoute(this));
