...
public abstract class AbstractInsertionStrategy implements InsertionStrategy {
  ...
  @Override
  public Collection<Job> insertJobs(Collection<VehicleRoute> vehicleRoutes,
  ...
  public abstract Collection<Job> insertUnassignedJobs(Collection<VehicleRoute> vehicleRoutes,
      Collection<Job> unassignedJobs);
  ...
  protected void insertJob(Job unassignedJob, InsertionData iData, VehicleRoute inRoute) {
    ...
    if (!(inRoute.getVehicle().getId().equals(iData.getSelectedVehicle().getId()))) {
      insertionsListeners.informVehicleSwitched(inRoute, inRoute.getVehicle(),
          iData.getSelectedVehicle());
    }
    for (Event e : iData.getEvents()) {
      eventListeners.inform(e);
    }
    insertionsListeners.informJobInserted(unassignedJob, inRoute, iData.getInsertionCost(),
        iData.getAdditionalTime());
  }
