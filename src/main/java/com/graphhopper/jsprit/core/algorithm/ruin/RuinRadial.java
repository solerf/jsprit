...
/**
 * RuinStrategy that ruins the neighborhood of a randomly selected job. The size and the structure
 * of the neighborhood is defined by the share of jobs to be removed and the distance between jobs
 * (where distance not necessarily mean Euclidean distance but an arbitrary measure).
 *
 * @author stefan
 */
public final class RuinRadial extends AbstractRuinStrategy {
   ...
   *
   * @param vrp
   * @param fraction2beRemoved i.e. the share of jobs to be removed (relative to the total number of
   *        jobs in vrp)
   * @param jobDistance i.e. a measure to define the distance between two jobs and whether they are
   *        located close or distant to eachother
   */
  public RuinRadial(VehicleRoutingProblem vrp, double fraction2beRemoved, JobDistance jobDistance) {
    ...
    JobNeighborhoodsImplWithCapRestriction jobNeighborhoodsImpl =
        new JobNeighborhoodsImplWithCapRestriction(vrp, jobDistance, noJobsToMemorize);
  ...
  public RuinRadial(VehicleRoutingProblem vrp, int noJobs2beRemoved,
  ...
  /**
   * Ruins the collection of vehicleRoutes, i.e. removes a share of jobs. First, it selects a job
   * randomly. Second, it identifies its neighborhood. And finally, it removes the neighborhood plus
   * the randomly selected job from the number of vehicleRoutes. All removed jobs are then returned
   * as a collection.
   */
  @Override
   ...
   */
  private Collection<Job> ruinRoutes(Collection<VehicleRoute> vehicleRoutes, Job targetJob,
