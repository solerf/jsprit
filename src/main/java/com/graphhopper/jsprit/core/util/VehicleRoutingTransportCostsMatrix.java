 ...
 * {@link com.graphhopper.jsprit.core.problem.cost.VehicleRoutingTransportCosts} in the
 * {@link com.graphhopper.jsprit.core.problem.VehicleRoutingProblem}.
 * <p>
 * Note that you can also use it with distance matrix only (or time matrix). But ones you set a
 * particular distance, this expects distance-entries for all relations. This counts also for a
 * particular time. If the method getTransportCosts(...) is then invoked for a relation, where no
 * distance can be found, an IllegalStateException will be thrown. Thus if you want to only use
 * distances only, do not use addTransportTime(...).
 *
 * @author schroeder
 */
public class VehicleRoutingTransportCostsMatrix
  ...
  public static class Builder {
    ...
    public Builder addTransportDistance(String from, String to, double distance) {
      ...
      if (distances.containsKey(key)) {
        log.warn(
            "distance from " + from + " to " + to + " already exists. This overrides distance.");
      }
    ...
    public Builder addTransportTime(String from, String to, double time) {
      ...
      if (times.containsKey(key)) {
        log.warn(
            "transport-time from " + from + " to " + to + " already exists. This overrides times.");
      }
      times.put(key, time);
      if (isSymmetric) {
        RelationKey revKey = RelationKey.newKey(to, from);
    ...
    if (!isSymmetric) {
      if (times.containsKey(key))
        ...
        throw new IllegalStateException(
            "time value for relation from " + fromId + " to " + toId + " does not exist");
    } else {
  ...
  public double getDistance(String fromId, String toId) {
    ...
    if (!isSymmetric) {
      if (distances.containsKey(key))
        ...
        throw new IllegalStateException(
            "distance value for relation from " + fromId + " to " + toId + " does not exist");
    } else {
  ...
  @Override
  public double getTransportCost(Location from, Location to, double departureTime, Driver driver,
      Vehicle vehicle) {
    if (vehicle == null)
      return getDistance(from.getId(), to.getId());
    VehicleCostParams costParams = vehicle.getType().getVehicleCostParams();
    return costParams.perDistanceUnit * getDistance(from.getId(), to.getId())
        + costParams.perTransportTimeUnit * getTime(from.getId(), to.getId());
  }
