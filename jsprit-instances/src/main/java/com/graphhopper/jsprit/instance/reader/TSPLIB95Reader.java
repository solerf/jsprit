...
public class TSPLIB95Reader {
  ...
  public void read(String filename) {
    ...
    while ((line_ = getLine(reader)) != null) {
      ...
      if (isCoordSection) {
        ...
        if (switchCoordinates) {
          coords[coordIndex] =
              Coordinate.newInstance(Double.parseDouble(tokens[2]), Double.parseDouble(tokens[1]));
        } else
          coords[coordIndex] =
    ...
    for (Integer depotId : depotIds) {
      VehicleTypeImpl type =
          VehicleTypeImpl.Builder.newInstance("type").addCapacityDimension(0, capacity).build();
      VehicleImpl vehicle = VehicleImpl.Builder
          .newInstance("vehicle").setStartLocation(Location.Builder.newInstance()
              .setId(depotId.toString()).setCoordinate(coords[depotId - 1]).build())
          .setType(type).build();
    ...
    for (Integer id_ : indexMap.keySet()) {
      ...
      Service service = Service.Builder
          .newInstance(id).setLocation(Location.Builder.newInstance().setId(id)
              .setCoordinate(coords[index]).setIndex(index).build())
    ...
    if (edgeType.equals("GEO")) {
      ...
      for (Job j : vrpBuilder.getAddedJobs())
        locations.add(((Service) j).getLocation());
      ...
      if (edgeWeightFormat.equals("UPPER_ROW")) {
        FastVehicleRoutingTransportCostsMatrix.Builder matrixBuilder =
            FastVehicleRoutingTransportCostsMatrix.Builder.newInstance(dimensions, true);
        int fromIndex = 0;
        int toIndex = 1;
        ...
        FastVehicleRoutingTransportCostsMatrix.Builder matrixBuilder =
            FastVehicleRoutingTransportCostsMatrix.Builder.newInstance(dimensions, false);
        int fromIndex = 0;
        int toIndex = 0;
        for (int i = 0; i < edgeWeights.size(); i++) {
          if (toIndex == dimensions) {
  ...
  private VehicleRoutingTransportCosts getGEOMatrix(List<Location> noLocations) {
    FastVehicleRoutingTransportCostsMatrix.Builder matrixBuilder =
        FastVehicleRoutingTransportCostsMatrix.Builder.newInstance(noLocations.size(), true);
